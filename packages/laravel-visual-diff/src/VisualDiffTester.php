...
class VisualDiffTester
{
    ...
    public function createDiffs(?string $url = null)
    {
        $this->preparePaths();
     ...
     */
    protected function createScreenshot(): void
    {
        ...
        if (! is_null(config('visualdiff.chrome_path'))) {
            $browsershot->setChromePath(config('visualdiff.chrome_path'));
        }
    ...
    protected function createDiff(?string $url = null)
    {
        ...
        if (! is_null($result)) {
            try {
                Assert::assertLessThanOrEqual(
                    ...
                    "See: " . rtrim($this->diffOutputPath , '/') . '/' . $this->getDiffFilename() . PHP_EOL .
                    ($url === null ? '' : ("Replicate: " . $url))
                );
