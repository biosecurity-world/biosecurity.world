@php use App\Services\NotionData\Models\Entry; @endphp
@php
    /* @var Entry $entry */
@endphp
...
<div class="entry flex flex-col h-full justify-between border-r border-t-0 rounded-r-3xl bg-white">
    <div>
        <div class="flex justify-between items-center bg-gray-50 pl-4 pr-6 py-4 rounded-tr-3xl border-b border-r">
            ...
            <button
                class="flex items-center duration-300 hover:bg-white hover:border-gray-200 border border-transparent transition p-2 -m-2 rounded-full group close-entry">
        ...
        @if ($entry->focusesOnGCBRs)
            <div class="px-4 py-1.5 bg-primary-50">
                <p class="flex items-center">
                 ...
                 This {{ $entry->nounForOrganizationType() }} focuses on <abbr
                            title="Global Catastrophic Biological Risks">GCBRs</abbr>.
                </span>
                </p>
            </div>
        @endif
        ...
        <div class="px-4 pb-2 mt-6">
            <h2 class="text-xl font-display font-bold">
                <a class="text-primary-600 underline" target="_blank" href="{{ $entry->link }}">
                    {{ $entry->label }}
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"
                         class="inline mb-3 size-4 text-gray-400 group-hover:text-primary-700"
            ...
            <ul class="mt-1.5 flex space-x-4 overflow-x-scroll">
                <li class="flex justify-center space-x-1.5">
                    @foreach($entry->activities as $activity)
                        <span style="background-color: {{ $activity->color->foreground() }}"
                              class="inline-block py-1.5 px-2.5 rounded-full">
